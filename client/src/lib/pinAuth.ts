...
export async function setPin(pin: string) {
  ...
  const derived = await crypto.subtle.deriveBits({ name: 'PBKDF2', salt, iterations: 200_000, hash: 'SHA-256' }, keyMaterial, 256);
  localStorage.setItem(PIN_HASH_KEY, toBase64(derived));
}

export function hasPin(): boolean { return !!localStorage.getItem(PIN_HASH_KEY); }
...
export async function clearPin() { localStorage.removeItem(PIN_HASH_KEY); localStorage.removeItem(PIN_SALT_KEY); }

export async function getEncryptionKeyFromPin(pin: string) { return deriveKeyFromPin(pin); }
